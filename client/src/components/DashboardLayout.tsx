...
export default function DashboardLayout({ children, demoMode = false }: DashboardLayoutProps) {
  ...
  const [activeItem, setActiveItem] = useState(location.startsWith("/app/calendar") ? "Calendar" : location.startsWith("/app/contacts") ? "Contacts" : location.startsWith("/app/deals") ? "Deals" : location.startsWith("/app/inbox") ? "Inbox & Text" : location.startsWith("/app/team") ? "Administration" : "Dashboard");

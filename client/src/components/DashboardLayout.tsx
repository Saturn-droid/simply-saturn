...
export default function DashboardLayout({ children, demoMode = false }: DashboardLayoutProps) {
  ...
  const [activeItem, setActiveItem] = useState(location === "/app/calendar" ? "Calendar" : "Dashboard");
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f0] text-[#212648]">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[17.5rem] flex-col overflow-hidden bg-[#20274b] px-3 py-4 text-white lg:flex">
        ...
        <nav aria-label="Workspace sections" className="relative mt-3 flex-1 space-y-1 overflow-y-auto pr-1">
          {navigationItems.map((item) => {
            ...
            const active = location === item.href || (!item.href && activeItem === item.label);
      ...
      <div className="min-h-screen lg:pl-[17.5rem]">
        ...
        <main className="relative p-4 sm:p-6 lg:p-8"><div aria-hidden="true" className="pointer-events-none absolute right-8 top-8 h-52 w-52 rounded-full bg-[#d1a467]/8 blur-3xl" /><div className="relative">{activeItem !== "Dashboard" && !location.startsWith("/app/calendar") ? <div className="mb-5 flex items-center gap-3 rounded-xl border border-[#171b39]/9 bg-white/70 px-4 py-3 text-xs text-[#616780]"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#eee9de] text-[#5a4c7a]"><Sparkles size={14} /></span><span><strong className="font-extrabold text-[#333958]">{activeItem}</strong> is represented in this app shell and ready for its connected product module.</span></div> : null}{children}</div></main>
      </div>
    </div>
  );
}

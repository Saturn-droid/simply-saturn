...
export default function Contacts() {
  ...
  const statusMutation = trpc.contacts.setStatus.useMutation({
    onSuccess: async (contact) => { setStatusDrafts((current) => ({ ...current, [contact.id]: contact.status ?? "" })); await utils.contacts.list.invalidate(); },
    onError: (error) => toast.error(error.message),
  });
  ...
  return <DashboardLayout demoMode>
    ...
    <section className="mt-7 overflow-hidden rounded-[1.35rem] border border-[#171b39]/9 bg-white shadow-[0_16px_45px_rgba(26,30,59,.06)]">
      <div className="flex flex-col gap-3 border-b border-[#171b39]/8 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <label className="flex items-center gap-2 text-xs font-extrabold text-[#686d82]"><span>Status</span><input list="contact-status-suggestions" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} aria-label="Filter contacts by status" className="w-32 rounded-lg border border-[#171b39]/10 bg-[#fbfaf6] px-2.5 py-2 text-xs text-[#454b69] outline-none focus:border-[#6a5889]" placeholder="All statuses" /><datalist id="contact-status-suggestions">{contactStatuses.map((status) => <option value={status} key={status}>{contactStatusLabels[status]}</option>)}</datalist></label>
      </div>
      ...
      {isAuthenticated && contacts.length > 0 ? <ContactListTable contacts={contacts} onOpenContact={(contactId) => setLocation(`/app/contacts/${contactId}`)} onStatusChange={(contactId, status) => statusMutation.mutate({ contactId, status })} /> : null}
    </section>
